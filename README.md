# Home Air Quality Monitor — PMS5003 + Raspberry Pi + Grafana

A tiny, RAM-frugal stack that reads a Plantower **PMS5003** particulate sensor over UART on a
**Raspberry Pi 3 B+ (1 GB RAM)**, stores every reading for 10 years, and serves live Grafana
dashboards — including a public, login-free wall display over a Cloudflare Tunnel.
...
Grafana + Docker risks OOM. VictoriaMetrics is a single ~30 MB binary that both scrapes and
...
| `setup.sh` | Installs exporter + VictoriaMetrics + Grafana + a 1 GB swapfile, all as systemd services |
| `grafana_setup.sh` | Provisions the datasource, two dashboards, a US-EPA AQI health banner, and an auto-cycling playlist (via the Grafana API) |
...
sudo bash setup.sh          # exporter + VictoriaMetrics + Grafana + swap
bash grafana_setup.sh       # dashboards + AQI banner + playlist  (Grafana on :3000)
...
Then open Grafana at `http://<pi-ip>:3000`. **Change Grafana's default password before
