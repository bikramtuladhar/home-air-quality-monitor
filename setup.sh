#!/usr/bin/env bash
# One-shot install: PMS5003 exporter + VictoriaMetrics + wall UI + swap.
...
echo ">>> [4/5] wall (replaces Grafana; removes it if present)"
if dpkg -l grafana >/dev/null 2>&1; then
  systemctl disable --now grafana-server || true
  apt-get purge -y grafana
  apt-get autoremove -y
  rm -f /etc/apt/sources.list.d/grafana.list
  rm -rf /var/lib/grafana /etc/grafana /etc/systemd/system/grafana-server.service.d
fi
cp "$HOME_DIR/wall.py" "$HOME_DIR/wall.html" /opt/pms5003/

cat >/etc/systemd/system/pm25-wall.service <<'EOF'
[Unit]
After=network.target
[Service]
ExecStart=/usr/bin/python3 /opt/pms5003/wall.py
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
systemctl enable --now pms5003 victoriametrics pm25-wall
...
echo "  Wall:            http://192.168.1.115:3000"
